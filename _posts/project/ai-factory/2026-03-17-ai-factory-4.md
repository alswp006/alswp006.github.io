...
flowchart TD
    ...
    Arch["Architecture ✦ NEW\ntypes.ts 생성\n빈 파일 스캐폴딩 (import 경로 확정)"]
    Scaffold["Scaffold\nTemplate Repo (auth + shadcn/ui + CLAUDE.md)\n디자인 레퍼런스 자동 주입 (6종 페이지 유형) ✦ NEW"]
    Coding["Coding Agent — Claude Code\n적응형: Haiku / Sonnet / Opus\n자기 검증 루프 (typecheck→test→build ×3) ✦ NEW"]
    Quality["Quality Gates\nTRUST-5 + fixLoop\nHaiku → Sonnet 에스컬레이션"]
    Deploy["Deploy\nRailway (웹) / EAS Build (모바일)"]

    Scout --> Planning --> Spec --> Arch --> Scaffold --> Coding --> Quality --> Deploy
