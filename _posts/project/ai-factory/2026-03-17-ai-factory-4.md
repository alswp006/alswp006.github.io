---
layout: post
title: Railway에서 exit code null과 싸운 밤 — AI Factory 개발기 (4)
...
# Railway에서 exit code null과 싸운 밤 — AI Factory 개발기 (4)
...
```
[Scout Agent]        뉴스 수집 → 아이디어 생성 (5중 필터)
   ↓
[Planning AI]        채팅 → PRD 생성 (10항목 검증)
   ↓
[Spec Agent]         PRD → SPEC → TASK → Work Packets (GPT-5.2, maxTokens 32000)
   ↓                 + Given/When/Then AC 강제
[Architecture]       types.ts 생성 + 빈 파일 스캐폴딩 (import 경로 확정)  ← NEW
   ↓
[Scaffold]           Template Repo (auth + shadcn/ui + CLAUDE.md)
   ↓                 + 디자인 레퍼런스 자동 주입 (6종 페이지 유형)  ← NEW
[Coding Agent]       Claude Code (적응형: Haiku/Sonnet/Opus)
   ↓                 + 자기 검증 루프 (typecheck→test→build ×3)  ← NEW
[Quality Gates]      TRUST-5 + fixLoop (haiku→sonnet 에스컬레이션)
   ↓
[Deploy]             Railway (웹) / EAS Build (모바일)

빌드 성공률: ~65% → ~80% (Phase 1 효과)
YELLOW 비율: ~30% → ~15%
```
