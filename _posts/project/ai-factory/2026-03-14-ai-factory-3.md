---
layout: post
title: 앱 하나 만드는 비용을 20배 줄인 이야기 — AI Factory 개발기 (3)
description: "패킷당 $2.58에서 $0.13으로 20배 절감. 6종 LLM 적재적소 배치, TDD 전략 진화, TRUST-5 품질 체계까지. AI Factory 최적화와 현재 모습을 정리합니다."
date: '2026-03-14 10:45:51 +0900'
categories:
    - project
    - ai-factory
tags: [AI Factory, 비용최적화, TDD, Claude Code, LLM]
...
# 앱 하나 만드는 비용을 20배 줄인 이야기 — AI Factory 개발기 (3)
...
[Scout Agent]        뉴스 수집 (GPT-4.1 mini) → 아이디어 생성 (Claude Sonnet)
   ↓                 5중 필터: 저작권 + 차별화 + 점수 + 필수필드 + 중복
[Planning AI]        채팅으로 아이디어 구체화 → PRD 생성 + 10항목 검증
   ↓
[Spec Agent]         PRD → SPEC → TASK → Work Packets (GPT-5.2)
   ↓                 Given/When/Then AC 강제, EARS 분류 (MoAI-ADK 참고)
[Template Repo]      ai-factory-template clone → Auth + DB + shadcn/ui 포함
   ↓
[Coding Phase]       Claude Code 단일 엔진 (Aider 제거)
   ↓                 적응형 모델 선택: Haiku(간단) / Sonnet(일반) / Opus(복잡)
   ↓                 test-alongside TDD + 패킷 간 코드 컨텍스트 전달 ($0)
[fixLoop]            tsc + lint + test 통합 수정 ×5회 (Haiku→Sonnet 에스컬레이션)
   ↓
[Re-planning Gate]   실패 원인 분류 → CODE_BUG / DESIGN_FLAW / SCOPE_TOO_LARGE
   ↓
[TRUST-5]            Tested + Readable + Unified + Secured + Trackable 정량 평가
   ↓
[Verification]       빌드 + 스모크 테스트 → README/CHANGELOG 자동 생성
   ↓
[Deploy]             웹: Railway / 모바일: EAS Build
   ↓
[Hotfix]             미니 파이프라인: Haiku 분석 → Sonnet 수정 → 재배포

모델 6종: Sonnet 4.6, Opus 4.6, Haiku 4.5, GPT-5.2, GPT-4.1 mini, Gemini 3.1 Pro
비용: 앱 1개 ~$2.87 (최적화 시 ~$2.10)
