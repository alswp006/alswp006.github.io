...
```mermaid
flowchart TD
    Scout["Scout Agent\n뉴스 수집 (GPT-4.1 mini)\n아이디어 생성 (Claude Sonnet)\n5중 필터: 저작권 + 차별화 + 점수 + 필수필드 + 중복"]
    Planning["Planning AI\n채팅으로 아이디어 구체화\nPRD 생성 + 10항목 검증"]
    Spec["Spec Agent (GPT-5.2)\nPRD → SPEC → TASK → Work Packets\nGiven/When/Then AC 강제\nEARS 분류 (MoAI-ADK 참고)"]
    Template["Template Repo\nai-factory-template clone\nAuth + DB + shadcn/ui 포함"]
    Coding["Coding Phase — Claude Code 단일 엔진\n적응형 모델: Haiku(간단) / Sonnet(일반) / Opus(복잡)\ntest-alongside TDD + 패킷 간 코드 컨텍스트 전달 ($0)"]
    Fix["fixLoop\ntsc + lint + test 통합 수정 ×5회\nHaiku → Sonnet 에스컬레이션"]
    Replan["Re-planning Gate\n실패 원인 분류\nCODE_BUG / DESIGN_FLAW / SCOPE_TOO_LARGE"]
    Trust["TRUST-5\nTested + Readable + Unified\n+ Secured + Trackable 정량 평가"]
    Verify["Verification\n빌드 + 스모크 테스트\nREADME · CHANGELOG 자동 생성"]
    Deploy["Deploy\n웹: Railway / 모바일: EAS Build"]
    Hotfix["Hotfix 미니 파이프라인\nHaiku 분석 → Sonnet 수정 → 재배포"]

    Scout --> Planning --> Spec --> Template --> Coding --> Fix --> Replan --> Trust --> Verify --> Deploy --> Hotfix

    subgraph info [" "]
        M["모델 6종: Sonnet 4.6 · Opus 4.6 · Haiku 4.5 · GPT-5.2 · GPT-4.1 mini · Gemini 3.1 Pro"]
        C["비용: 앱 1개 ~$2.87 (최적화 시 ~$2.10) | 플랫폼: Web (Next.js) + Mobile (Expo)"]
    end
```
