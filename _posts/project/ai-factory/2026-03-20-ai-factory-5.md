...
# 빌드 성공률 65%를 95%로 끌어올린 과정 — AI Factory 개발기 (5)
...
```
[Scout Agent]        뉴스 수집 → 아이디어 생성 (5중 필터 + 저작권 리스크 분석)
   ↓
[Planning AI]        채팅 → PRD 생성 (10항목 검증 + IP 검토)
   ↓
[Spec Agent]         PRD → SPEC → SPEC 검증 (Haiku 리뷰 → 수정)
   ↓                 → TASK → Work Packets
   ↓                 (Given/When/Then AC, EARS 분류, 파일 충돌 탐지)
[Architecture]       types.ts + 빈 파일 스캐폴딩 (import 경로 확정)
   ↓
[Scaffold]           Template Repo + CLAUDE.md + 디자인 레퍼런스 (6종)
   ↓                 + 이전 프로젝트 패턴 주입
[Coding Phase]       스마트 병렬 코딩 (독립 패킷 자동 감지)
   ├─ Tester (Sonnet)  AC 기반 테스트 먼저 작성 (TDD Red Phase)
   ├─ Coder (Sonnet/Opus)  테스트를 통과시키는 코드 작성 (Green Phase)
   ├─ Tester (Haiku)   실패 테스트 수정 (Refactor Phase)
   └─ 자기 검증 루프  typecheck → test → build (×3)
   ↓
[Quality Gates]      TRUST-5 + fixLoop (×5, 교착 감지)
   ↓                 + Re-planning Gate (설계 문제 → 재설계)
   ↓                 + 컨텍스트 맵 (모듈 의존성 그래프)
[Verification]       빌드 + 스모크 테스트 + 라이브 프리뷰
   ↓
[Deploy]             Railway (웹) / EAS Build (모바일)
   ↓
[Self-Heal]          배포 후 자동 에러 감지 → 핫픽스 → 재배포
