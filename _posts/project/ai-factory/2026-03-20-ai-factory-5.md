...
```mermaid
flowchart LR
    Scout["Scout"] --> Planning["Planning"]
    Planning --> Spec["Spec\nSPEC 검증 + Haiku 리뷰"]
    Spec --> Arch["Architecture\n빈 파일 스캐폴딩"]
    Arch --> Scaffold["Scaffold\nTemplate + 패턴 주입"]
    Scaffold --> Red["Tester Sonnet\nRed Phase"]
    Red --> Green["Coder Sonnet/Opus\nGreen Phase"]
    Green --> Refactor["Tester Haiku\nRefactor"]
    Refactor --> Check["자기 검증 ×3"]
    Check --> Gates["Quality Gates\nTRUST-5 + fixLoop"]
    Gates --> Deploy["Deploy\nRailway / EAS"]
