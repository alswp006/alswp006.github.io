---
layout: post
title: 빌드 성공률 65%를 95%로 끌어올린 과정 — AI Factory 개발기 (5)
description: "Tester Agent 분리, TDD red→green→refactor 3단계, SPEC 검증 패스, 자율 디버깅(self-heal), 스마트 병렬 코딩으로 빌드 성공률 95%, 완주 시간 50% 단축까지의 과정."
date: '2026-03-20 10:45:51 +0900'
categories:
    - project
    - ai-factory
tags: [AI Factory, TDD, 빌드성공률, 병렬코딩, self-heal]
...
```mermaid
flowchart TD
    Scout["Scout Agent<br/>뉴스 수집 → 아이디어 생성<br/>(5중 필터 + 저작권 리스크 분석)"]
    Planning["Planning AI<br/>채팅 → PRD 생성<br/>(10항목 검증 + IP 검토)"]
    Spec["Spec Agent<br/>PRD → SPEC → SPEC 검증 (Haiku 리뷰 → 수정)<br/>→ TASK → Work Packets<br/>(Given/When/Then AC, EARS 분류, 파일 충돌 탐지)"]
    Arch["Architecture<br/>types.ts + 빈 파일 스캐폴딩<br/>(import 경로 확정)"]
    Scaffold["Scaffold<br/>Template Repo + CLAUDE.md + 디자인 레퍼런스 (6종)<br/>+ 이전 프로젝트 패턴 주입"]
    Coding["Coding Phase<br/>스마트 병렬 코딩<br/>(독립 패킷 자동 감지)"]
    TDD_Red["Tester (Sonnet)<br/>AC 기반 테스트 먼저 작성<br/>TDD Red Phase"]
    TDD_Green["Coder (Sonnet/Opus)<br/>테스트를 통과시키는 코드 작성<br/>Green Phase"]
    TDD_Refactor["Tester (Haiku)<br/>실패 테스트 수정<br/>Refactor Phase"]
    SelfCheck["자기 검증 루프<br/>typecheck → test → build (×3)"]
    Quality["Quality Gates<br/>TRUST-5 + fixLoop (×5, 교착 감지)<br/>+ Re-planning Gate (설계 문제 → 재설계)<br/>+ 컨텍스트 맵 (모듈 의존성 그래프)"]
    Verify["Verification<br/>빌드 + 스모크 테스트 + 라이브 프리뷰"]
    Deploy["Deploy<br/>Railway (웹) / EAS Build (모바일)"]
    Heal["Self-Heal<br/>배포 후 자동 에러 감지 → 핫픽스 → 재배포"]

    Scout --> Planning --> Spec --> Arch --> Scaffold --> Coding
    Coding --> TDD_Red --> TDD_Green --> TDD_Refactor --> SelfCheck
    SelfCheck --> Quality --> Verify --> Deploy --> Heal
```
