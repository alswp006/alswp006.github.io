...
flowchart TD
    ...
    subgraph 설계
        Spec["Spec Agent\nSPEC 검증 + Work Packets"]
    end
    subgraph 준비
        Arch["Architecture\ntypes.ts + 빈 파일 스캐폴딩"] --> Scaffold["Scaffold\nTemplate + 디자인 레퍼런스"]
    end
    subgraph "TDD (red → green → refactor)"
        Coding["스마트 병렬 코딩\n독립 패킷 자동 감지"]
        TDD_Red["Tester (Sonnet)\nAC 기반 테스트 작성"]
        TDD_Green["Coder (Sonnet/Opus)\n테스트 통과 코드 작성"]
        TDD_Refactor["Tester (Haiku)\n실패 테스트 수정"]
        Coding --> TDD_Red --> TDD_Green --> TDD_Refactor
    end
    subgraph 품질
        SelfCheck["자기 검증 루프\ntypecheck + test + build"]
        Quality["Quality Gates\nTRUST-5 + fixLoop + Re-planning"]
        SelfCheck --> Quality
    end
    subgraph 배포
        Verify["Verification\n빌드 + 라이브 프리뷰"] --> Deploy["Deploy\nRailway / EAS Build"]
