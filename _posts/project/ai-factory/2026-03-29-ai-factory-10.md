...
<!-- 이미지: 대시보드 전체 화면 — 사이드바(아이디어, 채팅, 프로젝트) + 메인 영역이 보이는 스크린샷 -->
...
<!-- 이미지: 아이디어 목록 페이지 — 카드 형태로 아이디어들이 나열된 화면 -->
...
<!-- 이미지: 아이디어 상세 페이지 — "프로젝트로 전환" 버튼이 보이는 화면 -->
...
<!-- 이미지: 기획 채팅 화면 — AI가 질문하고 사용자가 답하는 형태 -->

아이디어를 프로젝트로 전환하면 자동으로 채팅 세션이 시작됩니다. Planning AI(Claude Sonnet)가 질문을 하면서 **PRD(Product Requirements Document)**를 만들어갑니다.
...
기획이 확정되면 Spec Agent(GPT-5.2)가 자동으로 돌아갑니다. **프로젝트 상세 페이지**에서 실시간으로 확인할 수 있습니다.

<!-- 이미지: 프로젝트 상세 페이지 — 진행 타임라인 + 이벤트 로그 -->

**이벤트 로그**에서 진행 상황이 실시간으로 표시됩니다.

```
[spec] SPEC 검증 통과
[spec] 13 work packets generated
[spec] === Done in 180s · $0.13 ===
```
...
```
12:34:01 [spec]      13 work packets generated — $0.13
12:34:15 [scaffold]  Template copied + npm install — $0.00
12:34:45 [tdd]       Packet 0001: 3 tests written (sonnet) — $0.12
12:35:20 [coding]    Packet 0001: Forcing sonnet for first packet — $0.65
12:36:02 [review]    Packet 0001: Score 74/100 (toss) — PASS
12:36:05 [quality]   Packet 0001: GREEN [코딩$0.65 수정$0.00 리뷰74점]
...
12:59:01 [deploy]    ait deploy completed ✅
```
...
다음 글에서는 실제로 만들어진 앱(RentCheck)의 품질을 보여드리고, 파이프라인으로 만든 앱이 정말 "쓸 만한지" 솔직하게 평가하겠습니다!!
